...
export function CatalogHud() {
  ...
  const fmtBRL = useCurrencyBRL();
  useCatalogIndex(); // memo global (prepara cache e tipos)
  return (
    <div
      style={{
        ...
        {(catalog as unknown as Array<{ id: string; name: string; price: number }>).map((item) => (
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              padding: 8,
              background: selected === item.id ? "#e0f2fe" : "#fff",
            }}
          >
            <div style={{ fontWeight: 600, fontSize: 12 }}>{item.name}</div>
            <div style={{ fontSize: 11, color: "#475569" }}>{fmtBRL(item.price)}</div>
          </button>
        ))}
      </div>
    </div>
